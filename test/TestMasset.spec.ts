...
contract("Masset", async (accounts) => {
    ...
    describe("initialize", async () => {
        ...
        context("should fail", async () => {
            it("when basket manager missing", async () => {
                await expectRevert(
                    masset.initialize("masset", "m", ZERO_ADDRESS),
            ...
            it("when already initialized", async () => {
                await masset.initialize("masset", "m", basketManagerObj.basketManager.address);
                await expectRevert(
                    masset.initialize("masset", "m", basketManagerObj.basketManager.address),
    ...
    describe("precision conversion", async () => {
        ...
        it("works both ways", async () => {
            ...
            expect(await getBalance(masset, standardAccounts.dummy1)).to.equal(
                "2000000000000000000",
            );
            expect(await getBalance(basketManagerObj.mockToken1, standardAccounts.dummy1)).to.equal(
                "0",
            ...
            await masset.transfer(standardAccounts.dummy2, "1000000000000000000", {
                from: standardAccounts.dummy1,
            });
            expect(await getBalance(masset, standardAccounts.dummy1)).to.equal(
                "1000000000000000000",
