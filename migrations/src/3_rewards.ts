...
export default async (
    ...
    ****************************************/
    ...
    const d_MetaToken = await conditionalDeploy(c_MetaToken, 'MetaToken', () => deployer.deploy(c_MetaToken, d_Nexus.address, default_, { from: default_ }));

    /***************************************
    2. StakingRewards
    ****************************************/

    const rewardsDistributor = await conditionalDeploy(c_RewardsDistributor, 'RewardsDistributor', () => deployer.deploy(c_RewardsDistributor, d_Nexus.address, [default_]));
