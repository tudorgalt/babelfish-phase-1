...
contract BasketManagerV4 is InitializableOwnable {
   ...
   */
    ...
    function checkBasketBalanceForDeposit(
        ...
        uint256 ratio = balance.mul(MAX_PROMIL).div(total);
    ...
    function getTargetRatio(address _basset) public view validBasset(_basset) returns(uint256) {
     ...
     */
    function addBasset(address _basset, int256 _factor, address _bridge, uint256 _min, uint256 _max, bool _paused, uint256 _targetRatio)
        public onlyOwner {
    ...
    function setRange(address _basset, uint256 _min, uint256 _max) public validBasset(_basset) onlyOwner {
        require(_min <= MAX_PROMIL, "invalid minimum");
        require(_max <= MAX_PROMIL, "invalid maximum");
    ...
    function setTargetRatio(address _basset, uint256 _targetRatio) public validBasset(_basset) onlyOwner {
        ...
        emit TargetRatioChanged(_basset, _targetRatio);
    }
